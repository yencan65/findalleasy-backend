...
async function handleVision(req, res) {
  const _visionEnabled = String(process.env.FAE_ENABLE_VISION || "").trim() === "1";
  if (!_visionEnabled) {
    return safeJson(res, { ok: false, disabled: true, error: "VISION_DISABLED" }, 501);
  }
  ...
  try {
    ...
    // Global paid-provider gate (SerpApi disabled by default)
    let _allowSerpLens = allowSerpLens;
    try {
      const _serpEnabled = String(process.env.FAE_ENABLE_SERPAPI || "").trim() === "1";
      if (!_serpEnabled) _allowSerpLens = false;
    } catch {}
