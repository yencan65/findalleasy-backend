// server/adapters/searchZurich.js
// ============================================================================
// ZURICH INSURANCE ADAPTER — S200 HARDENED (DRIFT-SAFE, ZERO-CRASH, NO-FAKE)
...
// ZERO DELETE: export isimleri korunur (searchZurich + aliases)
// ============================================================================
...
const PROVIDER_KEY = "zurich";
const ADAPTER_KEY = "zurich_insurance";
const PROVIDER_FAMILY = "insurance";
const BASE = "https://www.zurich.com.tr";
...
function detectCategory(title = "", desc = "") {
  ...
  if (/sağlık|health|tamamlayıcı|medikal|clinic|hospital/.test(t)) return "health_insurance";
  if (/kasko|trafik|araç|arac|otomobil|oto|motor|vehicle|car/.test(t)) return "car_insurance";
  if (/konut|ev|dask|home|property/.test(t)) return "home_insurance";
  if (/seyahat|travel|trip|flight/.test(t)) return "travel_insurance";
...
// MAIN ADAPTER — S200
// ---------------------------------------------------------
export async function searchZurich(query, regionOrOptions = "TR") {
  const t0 = Date.now();
...
// AdapterEngine uyumluluk
export const searchZurichScrape = searchZurich;
export const searchZurichAdapter = searchZurich;

export default { searchZurich };
