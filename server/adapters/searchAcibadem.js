// server/adapters/searchAcibadem.js
// ============================================================================
// ACIBADEM — S200 STANDARD FINAL (NO-FAKE • NO-CRASH • DRIFT-SAFE)
...
function stableId(url, i) {
  try {
    ...
    return stableIdS200(fixKey("acibadem"), String(url || ""), String(url || ""));
  }
}
...
export async function searchAcibadem(query = "", regionOrOptions = "TR") {
  const t0 = Date.now();
  const providerKey = fixKey("acibadem");
  const entryUrl = "https://www.acibademsigorta.com.tr/urunler";
  ...
  const useProxy =
    String(options.useProxy ?? process.env.FINDALLEASY_ACIBADEM_USE_PROXY ?? "1") !== "0" &&
  ...
  $(selectors.join(",")).each((i, el) => {
    try {
      ...
      if (!href) return;
      const fullUrl = abs("https://www.acibademsigorta.com.tr", href);
      ...
      const n = normalizeItemS200(candidate, providerKey, {
        ...
        baseUrl: "https://www.acibademsigorta.com.tr",
        region: String(region || "TR").toUpperCase(),
