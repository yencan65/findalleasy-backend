// server/adapters/searchAllianz.js
// ============================================================================
// ALLIANZ — S200 STANDARD FINAL (NO-FAKE • NO-CRASH • DRIFT-SAFE)
...
function stableId(url, i) {
  try {
    ...
    return stableIdS200(fixKey("allianz"), String(url || ""), String(url || ""));
  }
}
...
export async function searchAllianz(query = "", regionOrOptions = "TR") {
  const t0 = Date.now();
  const providerKey = fixKey("allianz");
  const entryUrl = "https://www.allianz.com.tr/urunler";
  ...
  const useProxy =
    String(options.useProxy ?? process.env.FINDALLEASY_ALLIANZ_USE_PROXY ?? "1") !== "0" &&
  ...
  $(selectors.join(",")).each((i, el) => {
    try {
      ...
      if (!href) return;
      const fullUrl = abs("https://www.allianz.com.tr", href);
      ...
      const n = normalizeItemS200(candidate, providerKey, {
        ...
        baseUrl: "https://www.allianz.com.tr",
        region: String(region || "TR").toUpperCase(),
...
export const searchAllianzScrape = searchAllianz;
export const searchAllianzAdapter = searchAllianz;

export default { searchAllianz };
