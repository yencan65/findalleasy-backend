// server/adapters/etsAdapter.js
// ============================================================================
// ETS (travel) — S200 HARDENED
// Scraping travel portals reliably is fragile (SPA + WAF + dynamic pricing).
// Policy:
// - STRICT (FINDALLEASY_ALLOW_STUBS=0): disabled until official affiliate/API.
// - DEV (FINDALLEASY_ALLOW_STUBS=1): SerpApi discovery (organic results; price null).
...
export async function searchEts(query, regionOrOptions = "TR") {
  ...
  if (!FINDALLEASY_ALLOW_STUBS) {
    const err = new Error("ETS disabled in STRICT mode (waiting official affiliate/API)");
  ...
  const key = getSerpKey();
  if (!key) {
    const err = new Error("SERPAPI_KEY missing for ETS discovery");
  ...
  if (!ok) {
    const err = new Error(`ETS discovery HTTP: ${status}`);
...
export const searchEtsScrape = async (q, opts = {}) => searchEts(q, opts);

// S200 wrapper
export async function searchEtsAdapter(q, opts = {}) {
  const started = now();
  const providerKey = SOURCE;

  try {
    const rawItems = await withTimeout(searchEts(q, opts), Number(opts.timeoutMs || 6500));
