// server/adapters/searchMapfre.js
// ============================================================================
// MAPFRE ADAPTER — S200 HARDENED (DRIFT-SAFE, ZERO-CRASH, NO-FAKE)
...
// ZERO DELETE: searchMapfre / searchMapfreScrape / searchMapfreAdapter export’ları korunur
// ============================================================================
...
const PROVIDER_KEY = "mapfre";
const ADAPTER_KEY = "mapfre_insurance";
const PROVIDER_FAMILY = "insurance";
const BASE = "https://www.mapfre.com.tr";
const LIST_URL = `${BASE}/sigorta/urunler`;
...
function _mkRes(ok, items, meta = {}) {
  ...
  // Back-compat: allow legacy code to treat response like array
  try {
...
// -----------------------------------
// CATEGORY (kept, hardened)
// -----------------------------------
function detectCategory(title = "", desc = "") {
  const t = (String(title) + " " + String(desc)).toLowerCase();

  if (/sağlık|health|medikal/.test(t)) return "health_insurance";
  if (/tamamlayıcı|supplement|complement/.test(t)) return "supplementary_health";
  if (/kasko|trafik|oto|arac|araç|otomobil|motor/.test(t)) return "car_insurance";
  if (/konut|ev|dask|home/.test(t)) return "home_insurance";
...
async function fetchHTML(url, signal, timeoutMs) {
  // proxy-first (preserved behaviour), but observable + timeout wrapped above
  try {
...
// MAIN ADAPTER — S200
// ============================================================================
export async function searchMapfre(query, regionOrOptions = "TR") {
  const t0 = Date.now();
  ...
  try {
    ...
    $(selectors.join(",")).each((i, el) => {
      ...
      // Insurance listing pages don't provide a price -> forced null (valid)
...
export const searchMapfreScrape = searchMapfre;
export const searchMapfreAdapter = searchMapfre;

export default { searchMapfre };
