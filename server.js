...
// ============================================================================
// ✅ CANONICAL SEARCH — TEK Otorite: router
// - /api/search için inline handler YOK. Drift'e sıfır tolerans.
// ============================================================================
...
// Mark as mounted for guards / auto-loader
getRouteRegistry(app).mounted.add("/api/search");
getRouteRegistry(app).mounted.add("/api/catalog");
getRouteRegistry(app).mounted.add("/api/csp");
    ...
    for (const spec of specs) {
      try {
        ...
        if (raw.startsWith("file://")) {
          u = new URL(raw);
          absPath = fileURLToPath(u);
        } else if (path.isAbsolute(raw)) {
          absPath = raw;
          u = pathToFileURL(absPath);
        } else {
          u = new URL(raw, import.meta.url);
          absPath = fileURLToPath(u);
        }
...
function registerInlineRoutes(appInstance) {
  ...
  // (Canonical /api/search router var; burada drift yaratma)
  // ---------------------------------------------------------------------------
...
async function main() {
  ...
  app.use((err, req, res, next) => {
    try {
      ...
      const statusRaw = Number(err?.status || err?.statusCode || 500);
      // product-info: barcode/kamera akisi PowerShell ve frontend'de 5xx ile patlamasin
      // (zaten route icinde try/catch var; buraya dusuyorsa beklenmedik bir hata vardir)
      const isProductInfo = url.startsWith("/api/product-info") || url.startsWith("/api/productInfo");
      const status = isProductInfo ? 200 : statusRaw;
